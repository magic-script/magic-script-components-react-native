...
class RectLayoutManagerImpl: RectLayoutManager {
    ...
    private fun layoutNodeWithinParentSize(node: Node, nodeBounds: Bounding, parentBounds: Bounding) {
        ...
        Log.d(TAG, "Parent bounds: left: ${nodeBounds.left}, bottom: ${nodeBounds.bottom}, right: ${nodeBounds.right}, top: ${nodeBounds.top}")
        ...
        val x = when (itemHorizontalAlignment) {
            Alignment.HorizontalAlignment.LEFT -> {
                parentBounds.left + nodeWidth / 2 + itemPadding.left
            }

            Alignment.HorizontalAlignment.CENTER -> {
                (parentBounds.left + parentBounds.right) / 2
            }

            Alignment.HorizontalAlignment.RIGHT -> {
                parentBounds.right - nodeWidth / 2 - itemPadding.right
            }
        }
        ...
        val y = when (itemVerticalAlignment) {
            Alignment.VerticalAlignment.TOP -> {
                parentBounds.top - nodeHeight / 2 - itemPadding.top
            }

            Alignment.VerticalAlignment.CENTER -> {
                (parentBounds.bottom + parentBounds.top) / 2
            }

            Alignment.VerticalAlignment.BOTTOM -> {
                parentBounds.bottom + nodeHeight / 2 + itemPadding.bottom
            }
        }
        Log.d("RectLayout", "New local Child position: x: $x, y: $y for vert align: $itemVerticalAlignment, horizon align: $itemHorizontalAlignment")
