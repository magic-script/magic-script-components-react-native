...
class SCNSpinnerCircle : SCNGeometry {
    fileprivate static let uTextCoordMultiplier: Float = 72.0

    var barBeginImage: UIImage? {
        didSet { materials[0].diffuse.contents = barBeginImage }
    }
    ...
    static fileprivate func createGeometry(size: CGSize, thickness: CGFloat) -> (sources: [SCNGeometrySource], elements: [SCNGeometryElement]) {
        ...
        let borderWidth: CGFloat = max(0, min(thickness, min(0.5 * size.width, 0.5 * size.height)))
        if borderWidth == 0 {
            let vertices = SCNSpinnerCircle.createVertices(size: size)
            var indices: [Int16] = [0]
