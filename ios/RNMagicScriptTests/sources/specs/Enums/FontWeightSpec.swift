...
class FontWeightSpec: QuickSpec {
    override func spec() {
        describe("FontWeight") {
            ...
            context("raw value") {
                it("should create proper object from raw value") {
                    objectByRawValue.forEach({ (arg0) in
                        let (key, value) = arg0
                        expect(FontWeight(rawValue: key)).to(equal(value))
                    })
                }
                ...
                it("should return nil for unknown font weight") {
                    expect(FontWeight(rawValue: "heavy")).to(beNil())
                }
            }
        }
    }
